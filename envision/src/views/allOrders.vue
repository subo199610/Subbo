<template>
    ...
    <div>
        <div v-for="(order,index) in ordersList" :key="index">
            <orders-con0 v-if="order.ordersState==0" :order="order" @getOrderInfo="getOrderInfo" @cancelOrder="cancelOrder" :url="url" ></orders-con0>
            <orders-con1 v-if="order.ordersState==1" :order="order" @getOrderInfo="getOrderInfo" :url="url"></orders-con1>
            <orders-con2 v-if="order.ordersState==2" :order="order" @getOrderInfo="getOrderInfo" :url="url" @takeOrder="takeOrder"></orders-con2>
            <orders-con3 v-if="order.ordersState==3" :order="order" @getOrderInfo="getOrderInfo" :url="url"></orders-con3>
            <orders-con4 v-if="order.ordersState==4" :order="order" @getOrderInfo="getOrderInfo" :url="url"></orders-con4>
            <orders-con5 v-if="order.ordersState==5" :order="order" @getOrderInfo="getOrderInfo" :url="url"></orders-con5>
        </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="orders_info">
                        ...
                        <ul class="orders_info_top">
                            ...
                            <li>收货时间：<span id="orders_end">{{orderInfo.orders_stime}}</span></li>
        ...
        <el-dialog
            ...
            <span slot="footer" class="dialog-footer btn-wrap">
                <button type="button" class="del-close" @click="AlertDialogVisible = false">取消</button>
                <button type="button" class="del-sure"  @click="cancelOrders(orderId)">确定</button>
            </span>
        </el-dialog>
        <el-dialog
                title="提示"
            ...
            <span>确定收到货物了吗？</span>
            <span slot="footer" class="dialog-footer btn-wrap">
                <button type="button" class="del-close" @click="TakeDialogVisible = false">取消</button>
                <button type="button" class="del-sure"  @click="takeOrders(orderId)">确定</button>
            </span>
        </el-dialog>
        <el-pagination
                background
...
<script>
    import ordersCon0 from '@/components/order/ordersCon0.vue'
    import ordersCon1 from '@/components/order/ordersCon1.vue'
    import ordersCon2 from '@/components/order/ordersCon2.vue'
    import ordersCon3 from '@/components/order/ordersCon3.vue'
    import ordersCon4 from '@/components/order/ordersCon4.vue'
    ...
    export default {
        ...
        methods:{
            ...
            getOrderInfo(data){
                ...
                this.$http.post('/user/getOrderInfo.do',this.$qs.stringify({
                    ...
                    switch (this.orderInfo.orders_state){
                        case 0 : this.orderInfo.state= '待付款';break;
                        case 1 : this.orderInfo.state= '待发货';break;
                        case 2 : this.orderInfo.state= '待收货';break;
                        case 3 : this.orderInfo.state= '待评价';break;
                        case 4 : this.orderInfo.state= '已评价';break;
                        case 5 : this.orderInfo.state= '已取消';break;
                    }
                }).catch((err)=>{
                    console.log(err)
                })
            ...
            cancelOrders(id){
                let uuid=JSON.parse(localStorage.getItem('userinfo')).uuid
                this.$http.post('/user/cancelOrders.do',this.$qs.stringify({
                    uuid:uuid,
                    ordersId:id
                })).then((res)=>{
                    console.log(res)
            ...
            takeOrders(id){
                let uuid=JSON.parse(localStorage.getItem('userinfo')).uuid
                this.$http.post('/user/takeOrder.do',this.$qs.stringify({
                    uuid:uuid,
                    ordersId:id,
                    ordersState:3
                })).then((res)=>{
